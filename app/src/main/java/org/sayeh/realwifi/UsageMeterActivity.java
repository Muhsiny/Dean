...
public class UsageMeterActivity extends Activity {
    ...
    private void initialiseHiddenDefaults(){
        ...
        if(pair.isEmpty()){
            pair=randomKey();
            e.putString("pair",pair);
        }
        if(p.getString("student","").isEmpty()){
            e.putString("student","Device-"+pair.substring(0,4));
        }
        e.apply();
    }
    ...
    private void dashboard(){
        screen();
        TextView header=title("WiFi Usage");
        header.setOnClickListener(v->tapAdmin());
        root.addView(header);
        TextView sub=body("مصرف اینترنت این دستگاه");
        sub.setOnClickListener(v->tapAdmin());
        root.addView(sub);

        if(!hasAccess()){
            renderDashboardValues(new Usage(new Totals(0,0),new Totals(0,0),new Totals(0,0),System.currentTimeMillis()),false);
            TextView note=small("برای نمایش آمار دقیق، دسترسی اندازه‌گیری را یک‌بار فعال کنید.");
            note.setGravity(Gravity.CENTER);
            root.addView(note);
            Button b=secondary("فعال‌سازی اندازه‌گیری");
            b.setOnClickListener(v->openAccess());
            root.addView(b);
            return;
        }

        TextView loading=small("در حال تازه‌سازی…");
        loading.setGravity(Gravity.CENTER);
        root.addView(loading);
        read(u->renderDashboard(u),e->{
            loading.setText("آمار در دسترس نیست. برای تازه‌سازی دوباره برنامه را باز کنید.");
        });
    }

    private void renderDashboard(Usage u){
        screen();
        TextView header=title("WiFi Usage");
        header.setOnClickListener(v->tapAdmin());
        root.addView(header);
        ...
        sub.setOnClickListener(v->tapAdmin());
        root.addView(sub);
        renderDashboardValues(u,true);
        TextView updated=small("آخرین تازه‌سازی: "+DateFormat.getTimeInstance(DateFormat.SHORT,new Locale("fa","AF")).format(new Date(u.at)));
        updated.setGravity(Gravity.CENTER);
        root.addView(updated);
    }

    private void renderDashboardValues(Usage u,boolean active){
        long day=shown("day",u.day.total());
        long week=shown("week",u.week.total());
        long month=shown("month",u.month.total());
        Totals daySplit=split(u.day,day);

        LinearLayout hero=card();
        hero.setOnClickListener(v->tapAdmin());
        TextView lab=small("امروز"); lab.setGravity(Gravity.CENTER); hero.addView(lab);
        TextView big=strong(active?fmt(day):"—");
        big.setTextSize(38); big.setGravity(Gravity.CENTER); big.setTextColor(0xFF0F766E); hero.addView(big);
        TextView flow=small(active?("دانلود  "+fmt(daySplit.rx)+"     آپلود  "+fmt(daySplit.tx)):"دانلود  —     آپلود  —");
        flow.setGravity(Gravity.CENTER); hero.addView(flow);
        root.addView(hero);
        ...
        View w=miniMetric("این هفته",active?fmt(week):"—");
        View m=miniMetric("این ماه",active?fmt(month):"—");
        row.addView(w,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f));
    ...
    private View miniMetric(String label,String value){
        LinearLayout c=card();
        TextView l=small(label); l.setGravity(Gravity.CENTER); c.addView(l);
        TextView n=strong(value); n.setTextSize(21); n.setGravity(Gravity.CENTER); c.addView(n);
        return c;
    }

    private void tapAdmin(){
        long now=System.currentTimeMillis();
        if(taps==0 || now-tapStart>2200){tapStart=now;taps=1;} else taps++;
        if(taps>=5){taps=0;openHiddenAdmin();}
    }

    private void openHiddenAdmin(){
        if(p.getString("pin","").isEmpty()) createFirstPin();
        else pinGate(this::adminHome);
    }
    ...
    private void createFirstPin(){
        ...
        EditText a=pinInput("PIN جدید، حداقل ۴ رقم");
        EditText b=pinInput("تکرار PIN");
        ...
        d.setOnShowListener(x->d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            String s=a.getText().toString(),t=b.getText().toString();
            if(s.length()<4 || !s.equals(t)){b.setError("PIN معتبر و یکسان وارد کنید");return;}
            p.edit().putString("pin",hashPin(s)).apply();
            d.dismiss(); adminHome();
        }));
        d.show();
    }
    ...
    private void pinGate(Runnable ok){
        EditText e=pinInput("PIN");
        LinearLayout box=new LinearLayout(this);box.setPadding(dp(20),0,dp(20),0);box.addView(e);
        ...
        d.setOnShowListener(x->d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            if(checkPin(e.getText().toString())){d.dismiss();ok.run();} else e.setError("PIN نادرست");
        }));
        d.show();
    }
    ...
    private void adminHome(){
        screen();
        root.addView(title("تنظیمات خصوصی"));
        if(!hasAccess()){
            root.addView(warn("دسترسی آمار مصرف هنوز فعال نیست."));
            Button a=primary("فعال‌کردن دسترسی");a.setOnClickListener(v->openAccess());root.addView(a);
            Button back=secondary("بازگشت");back.setOnClickListener(v->dashboard());root.addView(back);
            return;
        }
        TextView wait=body("در حال خواندن داده‌ها…");root.addView(wait);
        read(this::renderAdmin,e->wait.setText("خواندن آمار ممکن نشد."));
    }

    private void renderAdmin(Usage u){
        screen(); root.addView(title("تنظیمات خصوصی"));
        root.addView(section("آمار واقعی"));
        ...
        EditText d=numInput("امروز به MB — خالی = واقعی",overrideText("day"));
        EditText w=numInput("هفته به MB — خالی = واقعی",overrideText("week"));
        EditText m=numInput("ماه به MB — خالی = واقعی",overrideText("month"));
        root.addView(d);root.addView(w);root.addView(m);
        Button save=primary("ذخیره");save.setOnClickListener(v->{
            savePrev();saveOverride("day",d.getText().toString());saveOverride("week",w.getText().toString());saveOverride("month",m.getText().toString());log("مقادیر نمایشی تغییر کرد");renderAdmin(u);
        });root.addView(save);
        Button real=secondary("برگرداندن صفحه به آمار واقعی");real.setOnClickListener(v->{
            p.edit().remove("display_day").remove("display_week").remove("display_month").apply();log("نمایش به مقدار واقعی برگشت");renderAdmin(u);
        });root.addView(real);
        ...
        Button nameSave=secondary("ذخیره نام");nameSave.setOnClickListener(v->{
            String n=name.getText().toString().trim();if(!n.isEmpty())p.edit().putString("student",n).apply();toast("ذخیره شد");
        });root.addView(nameSave);

        String pair=p.getString("pair","");
        String code=reportCode(u);
        ...
        Button pin=secondary("تغییر PIN");pin.setOnClickListener(v->changePin());root.addView(pin);
        Button back=primary("بازگشت به آمار");back.setOnClickListener(v->dashboard());root.addView(back);
    }

    private void managerHome(){
        screen();root.addView(title("مجموع مصرف"));
        String ids=p.getString("mgr_ids","");long td=0,tw=0,tm=0;int count=0;
        if(!ids.isEmpty()) for(String id:ids.split("\\n")){
            if(id.trim().isEmpty())continue;count++;String k=safe(id);
            long d=p.getLong("mgr_"+k+"_day",0),w=p.getLong("mgr_"+k+"_week",0),m=p.getLong("mgr_"+k+"_month",0);
            td+=d;tw+=w;tm+=m;
            LinearLayout c=card();c.addView(strong(id));c.addView(small("امروز: "+fmt(d)+"   هفته: "+fmt(w)+"   ماه: "+fmt(m)));root.addView(c);
        }
        ...
        Button clear=secondary("پاک‌کردن فهرست");clear.setOnClickListener(v->new AlertDialog.Builder(this).setMessage("همه گزارش‌های ذخیره‌شده پاک شوند؟").setNegativeButton("نه",null).setPositiveButton("بلی",(x,y)->{
            SharedPreferences.Editor ed=p.edit();for(String key:p.getAll().keySet())if(key.startsWith("mgr_"))ed.remove(key);ed.remove("mgr_ids").apply();managerHome();
        }).show());root.addView(clear);
        Button back=secondary("بازگشت به تنظیمات خصوصی");back.setOnClickListener(v->adminHome());root.addView(back);
    }

    private void importDialog(){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(20),0,dp(20),0);
        EditText code=input("کد گزارش");EditText key=input("کلید اتصال");box.addView(code);box.addView(key);
        ...
        d.setOnShowListener(x->d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            Report r=parseReport(code.getText().toString().trim(),key.getText().toString().trim());
            if(r==null){code.setError("کد یا کلید معتبر نیست");return;}
            saveReport(r);d.dismiss();managerHome();
        }));d.show();
    }

    private String reportCode(Usage u){
        String id=p.getString("student","");
        String enc=Base64.encodeToString(id.getBytes(StandardCharsets.UTF_8),Base64.URL_SAFE|Base64.NO_WRAP|Base64.NO_PADDING);
        String body="WUM1|"+enc+"|"+u.at+"|"+u.day.total()+"|"+u.week.total()+"|"+u.month.total();
        return body+"|"+hmac(body,p.getString("pair",""));
    }

    private Report parseReport(String code,String key){
        try{
            String[] a=code.split("\\|");if(a.length!=7||!"WUM1".equals(a[0]))return null;
            String body=String.join("|",a[0],a[1],a[2],a[3],a[4],a[5]);
            if(!MessageDigest.isEqual(hmac(body,key).getBytes(StandardCharsets.UTF_8),a[6].getBytes(StandardCharsets.UTF_8)))return null;
            String id=new String(Base64.decode(a[1],Base64.URL_SAFE|Base64.NO_WRAP|Base64.NO_PADDING),StandardCharsets.UTF_8);
            return new Report(id,Long.parseLong(a[3]),Long.parseLong(a[4]),Long.parseLong(a[5]),Long.parseLong(a[2]));
        }catch(Exception e){return null;}
    }

    private String hmac(String body,String key){
        try{Mac m=Mac.getInstance("HmacSHA256");m.init(new SecretKeySpec(key.getBytes(StandardCharsets.UTF_8),"HmacSHA256"));return Base64.encodeToString(m.doFinal(body.getBytes(StandardCharsets.UTF_8)),Base64.URL_SAFE|Base64.NO_WRAP|Base64.NO_PADDING);}catch(Exception e){return "";}
    }

    private void saveReport(Report r){
        String ids=p.getString("mgr_ids","");boolean found=false;
        if(!ids.isEmpty())for(String x:ids.split("\\n"))if(x.equals(r.id))found=true;
        if(!found)ids=ids.isEmpty()?r.id:ids+"\n"+r.id;
        String k=safe(r.id);
        p.edit().putString("mgr_ids",ids).putLong("mgr_"+k+"_day",r.day).putLong("mgr_"+k+"_week",r.week).putLong("mgr_"+k+"_month",r.month).putLong("mgr_"+k+"_at",r.at).apply();
    }

    private boolean hasAccess(){
        AppOpsManager a=(AppOpsManager)getSystemService(APP_OPS_SERVICE);
        return a!=null&&a.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, Process.myUid(),getPackageName())==AppOpsManager.MODE_ALLOWED;
    }
    ...
    interface Ok{void done(Usage u);} interface Fail{void bad(Exception e);}
    ...
    private Totals query(long s,long e){
        try{
            NetworkStatsManager m=(NetworkStatsManager)getSystemService(NETWORK_STATS_SERVICE);
            if(m==null)return new Totals(0,0);
            NetworkStats.Bucket b=m.querySummaryForDevice(ConnectivityManager.TYPE_WIFI,null,s,e);
            if(b==null)return new Totals(0,0);
            return new Totals(b.getRxBytes(),b.getTxBytes());
    ...
    private void changePin(){
        ...
        EditText a=pinInput("PIN جدید");EditText c=pinInput("تکرار PIN");b.addView(a);b.addView(c);
        new AlertDialog.Builder(this).setTitle("تغییر PIN").setView(b).setNegativeButton("انصراف",null).setPositiveButton("ذخیره",(x,y)->{
            String s=a.getText().toString(),t=c.getText().toString();if(s.length()>=4&&s.equals(t)){p.edit().putString("pin",hashPin(s)).apply();toast("PIN تغییر کرد");}else toast("PIN معتبر و یکسان وارد کنید");
        }).show();
    }
