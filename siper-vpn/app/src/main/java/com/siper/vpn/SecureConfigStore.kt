...
class SecureConfigStore(context: Context) {
    private val prefs = context.getSharedPreferences("siper_secure", Context.MODE_PRIVATE)
    private val alias = "siper_wireguard_config_v1"
    ...
    fun save(config: String) {
        ...
        val encrypted = cipher.doFinal(config.toByteArray(StandardCharsets.UTF_8))
        prefs.edit()
