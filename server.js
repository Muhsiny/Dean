...
function mediaPublicUrl(pathValue){let p=String(pathValue||'');while(p.startsWith('/'))p=p.slice(1);return p?storagePublicBase+'/'+p.split('/').map(encodeURIComponent).join('/'):''}
